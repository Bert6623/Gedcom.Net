...
namespace Gedcom.Reports
{
	...
	public class GedcomGraphTest
	{
		...
		public void Test1()
		{
			...
			NUnit.Framework.Assert.IsTrue(graph.IsRelated(relative), "Expected Relationship not found", null);
			
			System.Console.WriteLine("Check 2");
			id = _reader.Parser.XrefCollection["I0684"];
			graph.Record = graph.Database[id];
			
			NUnit.Framework.Assert.IsTrue(!graph.IsRelated(relative), "Unexpected Relationship found", null);
