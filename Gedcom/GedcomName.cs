...
namespace Gedcom
{
	...
	public class GedcomName : GedcomRecord
	{
		...
		#endregion
		private static List<string> SuffixesL = null;
	...
	public void Split(string name)
	{
		...
		// Set results
		Given = name.Substring(0, pos).Trim();
		Surname = name.Substring(pos).Trim();
	}
	...
	private static bool IsSuffix(string s)
	{
		...
		return SuffixesL.Contains(sb.ToString().ToLower());
	}
	}
}
