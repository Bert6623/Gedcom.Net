...
public partial class admin_controls_admin_edit_themes : System.Web.UI.UserControl
{
        ...
        foreach (GridViewRow row in GridView1.Rows)
        {
            ...
            if (cb != null && cb.Checked)
            {
                try
                {
                    ...
                    ObjectDataSource1.DeleteParameters["Original_ThemeId"].DefaultValue = deleteSelected.ToString();
    ...
    #region Register Theme

    protected void btnInsert_Click(object sender, EventArgs e)
    {
        TextBox txtThemeUrlNew = GridView1.FooterRow.FindControl("txtThemeUrlNew") as TextBox;
        TextBox txtThemeThumbNew = GridView1.FooterRow.FindControl("txtThemeThumbNew") as TextBox;
        TextBox txtThemeNameNew = GridView1.FooterRow.FindControl("txtThemeNameNew") as TextBox;
        TextBox txtThemeDescriptionNew = GridView1.FooterRow.FindControl("txtThemeDescriptionNew") as TextBox;
        TextBox txtThemeCategoryNew = GridView1.FooterRow.FindControl("txtThemeCategoryNew") as TextBox;
        CheckBox cbxIsEnabledNew = GridView1.FooterRow.FindControl("cbxIsEnabledNew") as CheckBox;
        bool IsEnabled = false;
        if (cbxIsEnabledNew.Checked)
        {
            IsEnabled = true;
        }
        else
        ...
        ObjectDataSource1.InsertParameters["ThemeUrl"].DefaultValue = txtThemeUrlNew.Text.Trim();
        ObjectDataSource1.InsertParameters["ThemeThumbUrl"].DefaultValue = txtThemeThumbNew.Text.Trim();
        ObjectDataSource1.InsertParameters["ThemeName"].DefaultValue = txtThemeNameNew.Text.Trim();
        ObjectDataSource1.InsertParameters["ThemeDescription"].DefaultValue = txtThemeDescriptionNew.Text.Trim();
        ObjectDataSource1.InsertParameters["ThemeCategory"].DefaultValue = txtThemeCategoryNew.Text.Trim();
        ObjectDataSource1.InsertParameters["IsEnabled"].DefaultValue = IsEnabled.ToString();
        ...
        try
        {
            ...
            Msg.Text = "Theme REGISTRATION was successful.";
            Msg.Visible = true;
        }
        catch (Exception ex)
        {
